 ...
 * the weekly recap must not fire "during an active/paused ride, an SOS/emergency flow, a
 * storage-low or GPS-lost state, or any mid-task modal. The recap waits until the app is calmly
 * idle on Home." Neither platform enforced that — Android gated only on a pending B1 reveal.
 *
 ...
 *
 * This is the same defect class as TASK-116 (SOS ride earning a celebratory reveal) with a
 * different trigger, so the rule is named once here rather than inlined per call site.
 */
object CalmMomentGate {
     ...
     * @param isEmergencyActive an SOS/emergency flow is currently in flight.
