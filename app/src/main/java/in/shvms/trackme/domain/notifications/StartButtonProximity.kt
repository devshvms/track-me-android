 ...
 * someone 20 minutes short who typically rides 40, and stays quiet for someone 20 minutes short who
 * typically rides 10 — where the honest sentence would be "this one does not, quite".
 *
 * Pure so both platforms hold the same rule.
 */
object StartButtonProximity {

    /**
     * The line to show under the start button, or null when there is nothing true to say.
     *
     * @param minutesToNextLevel remaining lifetime active minutes, or null at the maximum level.
     * @param typicalActiveMinutes the rider's median active minutes, from [RideHistoryProfile].
     *   Null when there is no history — a first-time rider gets no prediction, because the app has
     *   nothing to base one on and inventing a default would be the app guessing about a stranger.
     */
    fun line(
        minutesToNextLevel: Long?,
        nextLevelName: String?,
        typicalActiveMinutes: Long?,
    ): Line? {
        if (minutesToNextLevel == null || nextLevelName.isNullOrBlank()) return null
        if (typicalActiveMinutes == null || typicalActiveMinutes <= 0) return null
        ...
        if (minutesToNextLevel > typicalActiveMinutes) return null
        return Line(minutesToNextLevel, nextLevelName)
    }

    data class Line(val minutes: Long, val levelName: String)
}
