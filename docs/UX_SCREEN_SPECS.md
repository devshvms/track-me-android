...
| 4 | State preservation | ⚠️ | **Code-fixed in the Android follow-up after `26eb57e`:** `TrackingService` persists an active-session marker and restores the unfinished ride, path, persona, duration, distance, and current speed after sticky-service/process recreation; `MainActivity` also requests reattachment when the user relaunches. Unmarked orphaned rides retain the existing localized recovered/discarded Snackbar fallback. Physical process-death, force-stop, relaunch, and HUD continuity checks remain under TASK-005. |
