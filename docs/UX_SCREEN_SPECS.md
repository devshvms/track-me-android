...
| 3 | Accessibility | ✅ code | **Fixed in `b9599c4`:** `RadialStartRideButton` now exposes a semantic Start Ride button, activity-selection state, and custom TalkBack actions for Walk/Run/Cycling/Bike/Car while preserving the touch drag gesture. Physical TalkBack walkthrough remains a device check. |
...
4. Run TalkBack through: Home → start a ride via the persona picker → Settings → EmergencySetup → add a contact → RideDetail → scrub the chart (currently unlabeled per above). This walkthrough still has known failures in RideDetail chart semantics and EmergencySetup form labels; the Home persona actions are covered in source and need physical-device confirmation.

## Sign-off status

Per-screen specs above were the completed W3 deliverable (spec side) as of 2026-07-17; **re-synced 2026-07-18 against Codex's landed commits** (`49c7d12`, `63047ba`, `f315d25`, `26eb57e`, `8d59caf`, `2695ec5`, `b9599c4`) — all former P0 code gaps now have fixes in place, pending runtime/device verification. Home #3 persona-picker semantics are code-complete; RideDetail #3 chart semantics and EmergencySetup #3 form labels remain the largest open W3 scope. 🔍 items require Codex or the user to execute the device checklist below; they cannot be closed by code review alone.
