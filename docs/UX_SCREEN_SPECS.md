...
| 3 | Accessibility | ✅ code | **Fixed in `b9599c4`:** `RadialStartRideButton` now exposes a semantic Start Ride button, activity-selection state, and custom TalkBack actions for Walk/Run/Cycling/Bike/Car while preserving the touch drag gesture. Physical TalkBack walkthrough remains a device check. **Minor follow-up (P2, code-review only, not blocking):** the inner `Icon` at line 354 still carries its own `contentDescription = "Start Ride"`; with `mergeDescendants = true` on the parent this may get concatenated onto the parent's description during announcement. Worth confirming on-device and dropping the inner description if it reads as duplicated. |
