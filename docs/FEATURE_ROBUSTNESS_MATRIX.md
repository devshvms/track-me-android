...
| GPS signal gap mid-ride (tunnel, urban canyon) | User sees a live "GPS lost" indicator while recording continues; gap is later shown clearly in ride detail | Gap *detection* only happens post-ride in `GPSProcessor.kt:101-118` (chunks split >15s) and is rendered in the `RideDetailScreen` chart (>25s threshold, lines 968-988). **Live-time indicator is dead code**: `TrackingState.GPS_LOST` (`TrackingService.kt:28`) is never set, and `ActiveRideHudPanel`'s `timeSinceLastGps` parameter is computed and passed in but never read in the composable body. A rider gets zero in-ride feedback that GPS has dropped. | ❌ | **P0** |
| Airplane mode / GPS toggled off mid-ride | Clear in-app state ("GPS unavailable — check location settings") | No `LocationSettingsRequest`/`SettingsClient`/`isLocationEnabled` check anywhere in the codebase. Combined with the dead GPS-loss indicator above, this is a fully silent failure — `requestLocationUpdates` simply stops delivering callbacks and the UI shows nothing. | ❌ | **P0** |
...
| Device has no accelerometer / linear-accel sensor | Auto-pause feature degrades gracefully or is disabled; tracking still records distance/speed | `MotionSensorManager.kt:25-26,99-101`: if neither sensor exists, `startListening()` no-ops and `isDeviceStationary()` always returns `true` (energy never updates from 0). With `intelligent_auto_pause` on by default, `effectiveSpeed` is forced to `0f` permanently in `TrackingService.kt:90-95` — **distance/speed would never accumulate on such hardware.** Not detected, not flagged, no fallback. | ❌ | **P0** (if any target device lacks the sensor; unverified how common this is in our device matrix) |
...
| Failure condition | Expected | Actual (audited) | Status | Severity |
|---|---|---|---|---|
| SOS triggered — notification to user that it's in progress/sent | User sees confirmation the alert was sent (or failed) | The dedicated `sos_channel` (`IMPORTANCE_HIGH`) is **created but never posted to anywhere in the codebase**. `EmergencyBroadcastWorker` has no UI surface and posts no notification when broadcasting. A user who triggers SOS gets no system confirmation that anything happened. | ❌ | **P0 — this is a safety feature that fails silently** |
| SMS send fails (no signal, carrier reject, permission revoked) | User/app knows the alert didn't go out and can retry or is told to call emergency services directly | `EmergencyBroadcastWorker.broadcast()` wraps each `sendTextMessage` in generic try/catch that only logs via `errorLogger` (lines 156-166) — no user-facing failure state at all. No delivery-confirmation `PendingIntent` is passed (`sendTextMessage(..., null, null)`, line 158), so even a "sent successfully" case can't be distinguished from "handed to radio but never delivered." | ❌ | **P0** |
...
| Sync failure reporting accuracy | `SyncResult` reflects what actually happened | **Bug**: `syncAll()`/`syncPeriodic()` increment `uploaded++` unconditionally after calling `uploadRideInternal()`, but that function swallows its own exceptions internally (catches, logs, returns without rethrowing — lines 296-299). A partially-failed sync can report `SyncResult.Success` with an inflated count. | ❌ | **P0 — user-facing sync status is untrustworthy** |
| Background periodic sync via WorkManager | `WorkManager` retries on real failure | **Bug**: `SyncWorker.doWork()` calls the non-suspend `syncPeriodic()`, which internally does `syncScope.launch{}` (fire-and-forget on its own scope). `doWork()` returns `Result.success()` and writes `last_sync_time` immediately, without ever awaiting the actual sync outcome. `Result.retry()` (line 22) is dead code — no exception can reach it. | ❌ | **P0 — periodic background sync cannot be trusted to actually complete or retry** |
