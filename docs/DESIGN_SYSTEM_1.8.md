...
### 4.3 Generation — and what is actually approximate about it

*Resolved in phase 5. This section previously said "regenerate with Material Color Utilities
before 1.8.0 ships"; that turned out to be the wrong question.*
...
**The tone axis is not an approximation at all.** HCT's tone *is* CIELAB L* — the two systems
differ in how they model hue and chroma, not lightness. `TonalRampAccuracyTest` measures the L* of
every one of the 80 ramp entries against the tone in its name:

| | |
|---|---|
| Worst deviation across all 80 entries | **0.21 tone points** |
| 8-bit sRGB quantisation, midtones | ~0.2 tone points |
| Test tolerance | 0.5 |

In other words the ramps are as accurate as sRGB can represent. The same test also asserts each
ramp is monotonic in lightness, which is the failure a contrast test cannot see: a single inverted
pair silently breaks every "one step lighter" decision in the scheme without changing any measured
ratio.

What genuinely remains approximate is **chroma above T70**, where gamut mapping bites — a CAM16
chroma of 48 and a CIELCh chroma of 48 are not the same quantity, so the primary ramp desaturates
slightly earlier than Material's generator would take it. That is a difference in saturation, not
in lightness or ordering, and `ThemeContrastTest` measures every pair the app actually renders.

**So regeneration is no longer an open item.** It would move a handful of high-tone values by a few
points of chroma, and it is not worth re-tuning a palette that is measured correct on both axes
anyone reasons about. If the brand hue ever changes, edit `SEED_HUE` and regenerate with whichever
tool — both tests re-validate whatever values are present.

---

## 5. The motion system

Seven tokens, split along one rule that prevents most motion bugs:
...
| `spatialBounded` | **1.0** | 700 | Drag release, clamped offsets, bounded fills |
...
**`spatialBounded` is the seam**: real movement, but clamped, so it must not overshoot either.

A value that overshoots past a bound clips. For alpha that is a visible flash; for a clamped drag
offset it is a thumb drawn outside its container and cut off, which reads as a rendering bug
rather than as physics. `MotionTokenAdoptionTest` asserts the damping ratios directly, in both
schemes, so the split cannot decay into decoration.

Springs replace tweens because a tween commits to a duration: interrupt it mid-flight and it
restarts or snaps. A spring carries velocity through the interruption.

### 5.1 Adoption (phase 5)

Master ran **30** `tween` calls against 4 springs. *(An earlier draft of this document said 39 —
that count matched `between(` as well, including `computeDistanceBetween`, and was wrong.)*

**23 of the 30 are now motion tokens.** The seven that are not are exempt for reasons that are
type-level or physical, not stylistic, and `MotionTokenAdoptionTest` fails the build if any *other*
file calls `tween`:

| Remaining | Why it cannot be a spring |
|---|---|
| 4 × `infiniteRepeatable` | Requires a `DurationBasedAnimationSpec`. A spring has no duration. |
| 2 × launch pulse (`RadialStartRideButton`) | Duration-**coupled** pair. The ring must reach full size at the instant it reaches zero opacity; spring settle time depends on distance travelled, and these travel 0.65 of scale against 0.45 of alpha, so no stiffness keeps them together. A shared duration is what expresses the constraint. |
| 1 × stop slide (`ActiveRideHudPanel`) | Timed **choreography**: the slide, the 350ms the acknowledgement stays readable, and the commit are one sequence whose total is what the rider experiences as "how long stopping takes". A spring settles as a function of travel distance, which here is the screen width — so the same gesture would stop the ride at a different moment on a tablet than on a phone. |

The last one was found by a test rather than by reading, which is the useful part: converting it
pushed `onStopRide()` from 500ms after the gesture to roughly 880ms, and
`RideControlAccessibilityComposeTest` — which drives the clock manually and asserts the commit has
happened by 600ms — failed. Nothing about the code looked wrong.

**The same conversion exposed a second-order bug worth knowing about.** `Animatable`'s default
visibility threshold is 0.01, which assumes a normalised 0..1 value. `dragOffset` is in *pixels*,
so the default asked every spring to land within a hundredth of a pixel — several hundred
milliseconds of decay that finished looking finished long before it finished. Pixel-valued
`Animatable`s need an explicit `visibilityThreshold = 1f`.

The exemption list is data in the test, not a comment — adding to it is a deliberate act, and a
second test fails if an entry on it no longer has a `tween` behind it, so stale permissions cannot
accumulate.

### 5.2 Material 3 Expressive: still correctly deferred

The plan was always "adopt Expressive **if** `material3:1.5.0` is stable". Checked against the
Google Maven index in August 2026: the newest published version is **`1.5.0-alpha26`** — no beta,
no rc, no stable. The condition has not fired.

This is the dependency-inversion argument paying off rather than failing. Every animated component
in the app now depends on `TrackMeMotionScheme`, and not one line of it references Material
Expressive. When 1.5.0 does stabilise, `Standard` is re-backed by the real `MotionScheme` and no
screen changes.
...
All five phases land on the single branch `feat/1.8.0-design-system-foundations` and ship as one
...
### 11.3 Phase 5, in full

The phase that closed the gap between what this document described and what the app actually did.

- **Motion adoption** — 24 of 30 `tween` call sites now read the scheme; the six that remain are
  exempt for type-level or physical reasons, enforced by a test. A seventh token, `spatialBounded`,
  was added because converting the drag slider surfaced a category the original six could not name
  honestly (§5.1).
- **Colour verified rather than regenerated** — measuring the ramps showed the tone axis is exact
  to 0.21 points, so the open "regenerate with Material Color Utilities" item was the wrong
  question and is now closed with a test instead (§4.3).
- **Expressive checked, still deferred** — `material3:1.5.0` remains at `alpha26` (§5.2).
- **Deprecation debt cleared** — the build emitted eight deprecation warnings for most of this
  branch, including one I twice reported as fixed when it was not. All are now gone: three
  direction icons moved to `AutoMirrored`, a hand-rolled vibrator call that predated the shared
  helper was deleted in favour of it, and a `MediaCodec` constant unreachable above API 21 was
  removed from the export drain.

---

## 12. Known limitations of this branch

*Updated at the end of phase 5. The phase-1 wording — "never run on a device" — no longer holds:
...
- **No screenshot tests.** Verification is a person looking at a build. Every visual claim in this
  document rests on that, and nothing in CI would catch a regression in any of it. This is the
  single largest remaining gap, and it is the reason the motion conversion in §5.1 — which changed
  the feel of two dozen animations at once — needed a human to sign it off.
...
- **Spring timings were chosen, not measured against the old durations.** The tokens replace
  durations of 80–420ms with stiffnesses that settle in roughly 100–250ms, so most converted
